...
public class RegisterActivity extends AppCompatActivity {
                        ...
                        introAPI.signUp(id, pw, name, number, email, sos).enqueue(new Callback<CheckSuccess>() {
                            ...
                            public void onResponse(Call<CheckSuccess> call, Response<CheckSuccess> response) {
                                ...
                                if (response.isSuccessful() && response.body() != null) { // 성공적으로 받아왔을 때
                                    ...
                                    Toast.makeText(getApplicationContext(), "회원가입 도중 오류가 발생했습니다. 다시 시도해주세요.", Toast.LENGTH_LONG).show();
    ...
    public void findViewById() {
        ...
        register_pw.setOnFocusChangeListener(new View.OnFocusChangeListener() { // EditText 영역을 입력하고 다른 영역으로 이동할 때 리스너 발생
            @Override
            public void onFocusChange(View view, boolean hasFocus) { // 변화가 감지됐을 때
                String pw = register_pw.getText().toString();
                String pw2 = register_pw2.getText().toString();
                Matcher matcher = VALID_PASSWOLD_REGEX_ALPHA_NUM.matcher(pw); // 기존에 설정했던 제한과 입력한 패스워드 비교
